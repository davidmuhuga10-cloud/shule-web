-- 0030_report_card_aggregates_2dp.sql
-- Sprint Review correction (final): the individual-vs-aggregate rounding
-- rule is broader than "Mean Marks keeps 2dp" — only an INDIVIDUAL result
-- (one subject's own score for one student, e.g. the per-subject 'score'
-- field below) rounds to a whole number. Every AGGREGATE/computed figure
-- (Total, Average) keeps 2 decimal places wherever it appears, matching the
-- JS-side fix already applied to src/views/_reportCard.mjs. The overall
-- grade's band lookup still compares round(v_average) (a whole number)
-- against the integer grade bands so a fractional average can't fall
-- through a gap between bands (same fix as migration 0029's per-subject
-- lateral join) — only the RETURNED average value itself is now 2dp.
-- Supersedes migrations/0029_grade_report_card_rounded.sql (that fix is
-- carried forward unchanged below; only 'total' and 'average' changed).
