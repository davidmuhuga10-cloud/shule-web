-- 0067_report_card_pathway_and_balance.sql
-- Report Forms fix (two independent, unrelated additions bundled together
-- because both are needed by the same printed report):
--
-- 1) get_report_card() already fetches the student's stream full row
--    (v_stream) but never returns v_stream.pathway to the client, so the
--    front end (_reportCard.mjs) has had no way to know whether a given
--    student's own stream is actually pathway-bearing (only Senior
--    Secondary streams get a `pathway` value at creation — see
--    classes.mjs/academics.mjs). It was instead inferring "this student is
--    on a pathway" purely by keyword-matching subject NAMES against a
--    fixed list (clusterSummaryHtml()/CLUSTER_KEYWORDS in _reportCard.mjs),
--    which spuriously matches common subjects (Math, English, Science...)
--    at every grade level — so the pathway summary was showing up for
--    students who have no pathway at all. Exposing 'stream_pathway' lets
--    the client gate correctly: show the pathway summary only when BOTH the
--    school-wide toggle is on AND this student's own stream has a pathway.
--    Body below is copied verbatim from 0030_report_card_aggregates_2dp.sql
--    (the current latest definition — confirmed via grep across every
--    migration) with exactly one addition to the final jsonb_build_object.
--
-- 2) A new report_card_fee_balance() RPC for the optional "fee balance"
--    box on the printed Report Form. finance_student_balance() (in
--    0031_finance_module.sql) cannot be reused directly here: it requires
--    finance_can_collect() (finance_manage_fees / finance_record_collections
--    capability), which ordinary teachers, students and parents viewing a
--    report card do not hold and should not need — a parent must be able to
--    see their OWN child's balance on the report form without being granted
--    finance access. This new function instead reuses get_report_card()'s
--    own authorization rule (admin/teacher, the student themself, or that
--    student's parent) and returns only the single headline 'balance'
--    figure — never the full vote-head breakdown finance staff see.
...
begin
  ...
  return jsonb_build_object(
    'student', jsonb_build_object(
      ...
      'gender', v_student.gender, 'stream_pathway', coalesce(v_stream.pathway, '')
    ),
...
-- ----------------------------------------------------------------------------
-- report_card_fee_balance() — the headline fee balance for the optional
-- Report Form box, gated by the SAME viewer authorization get_report_card()
-- already uses (admin/teacher, the student themself, or their parent) rather
-- than finance_can_collect(), so a parent/student can see this figure on
-- their own report without holding a finance capability. Deliberately
-- returns only the single 'balance' number (positive = owing, negative =
-- credit/overpayment) — never the vote-head breakdown finance_student_
-- balance() exposes to finance staff.
-- ----------------------------------------------------------------------------
create or replace function public.report_card_fee_balance(p_student_id uuid)
returns numeric
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if not exists (select 1 from public.students where id = p_student_id and school_id = v_caller_school) then
    raise exception 'Student not found';
  end if;

  select id into v_bf_id from public.finance_vote_heads where school_id = v_caller_school and code = 'BALANCE_BF';
  ...
  with expected_by_vh as (
    ...
    where vh.school_id = v_caller_school
  ),
  paid_by_vh as (
  ...
  select coalesce((select sum(e.expected - e.credit_note - p.paid) from expected_by_vh e join paid_by_vh p on p.vote_head_id = e.vote_head_id), 0)
    - (select amount from unallocated)
  into v_balance;

  return coalesce(v_balance, 0);
end;
$$;
grant execute on function public.report_card_fee_balance(uuid) to authenticated;
