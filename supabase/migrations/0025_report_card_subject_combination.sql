-- 0025_report_card_subject_combination.sql — Round 5 §3: "when subjects are
-- combined (Subject Combination), the combined result should be what's
-- shown on the Report Form — deviation included — treated exactly as a
-- single subject, the same way it's already handled correctly on the Mark
-- List." Round 2 §3 deliberately left get_report_card() untouched (a scope
-- decision documented at the time) — this migration is that reversal.
--
-- Mirrors getBroadsheet()'s own combo-folding exactly (src/lib/api/
-- results.mjs): a combo only "activates" for THIS report card when every
-- one of its member subjects is actually present in what this caller can
-- see (per_row's own WHERE clause already tells staff from
-- student/parent apart — staff sees any entered subject, a student/parent
-- only a published one — so re-using `per_subject`, which is built from
-- per_row, automatically respects that same distinction). An active
-- combo's members are folded into ONE synthetic entry — id
-- 'combo:<subject_combinations.id>', matching the exact string
-- getBroadsheet() already uses for the same combo — whose score is the
-- weighted sum of its members' effective scores, graded like any other
-- subject, and counted as ONE subject everywhere below (total, average,
-- overall grade, the minimum-subjects/X-grade rule). classAvgBySubject on
-- the Report Forms screen (reportForms.mjs) is keyed off getBroadsheet()'s
-- own subject ids, so matching that same 'combo:<id>' string is what makes
-- the Dev. (vs Class) column work correctly for a combined subject too.
