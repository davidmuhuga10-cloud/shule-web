-- 0028_round_report_card.sql
-- Round 6 §1 (BUG): the Report Form (get_report_card RPC) was still
-- showing 2-decimal-place marks for combined subjects (Subject
-- Combinations like SST/CRE) and for plain subjects' own raw marks —
-- "no subject should ever report marks as a decimal". Same fix as
-- getBroadsheet() in src/lib/api/results.mjs (which the Mark List
-- reads), applied here since this RPC computes the Report Form's
-- figures independently. Purely a display change — nothing rounded
-- here ever fed into ranking (the cohort/ranked CTEs inside this
-- function compute their own separate, unrounded totals for that).
