 ...
 * accounts don't have.
 */
import { esc, options, toast, confirmAction, modal, closeModal, renderPrereq, loader } from '../app.js';
...
async function loadLinks(root, students) {
  ...
  listEl.innerHTML = `<div class="table-wrap"><table class="data">
    ...
    <tbody>${links.map((l) => `<tr>
      <td>${esc(l.parent_name)}<div class="muted" style="font-size:12px">${esc(l.parent_email || '')}</div></td>
...
function openProvisionModal(root, students) {
  modal({
    ...
    onOk: async () => {
      ...
      if (!full_name || !phone) { toast('Name and phone number are required.', 'err'); return; }
      const res = await Db.parents.provision({ full_name, phone });
      if (!res.ok) { toast(res.message, 'err'); return; }
      closeModal();
      if (res.alreadyProvisioned) {
        toast('This parent already has an account.', 'warn');
      } else {
        toast(`Parent account created — login: ${res.email}, default password: ${res.defaultPassword}`, 'ok');
      }
      loadParents(root);
    }
  });
