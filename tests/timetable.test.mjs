...
const BASE_TABLES = {
  ...
  subject_class_assignments: [
    { id: 'sca1', subject_id: 'math', class_id: 'c1', stream_id: 'str1', periods_per_week: 5, is_double: false },
    { id: 'sca2', subject_id: 'eng', class_id: 'c1', stream_id: 'str1', periods_per_week: 4, is_double: false },
    { id: 'sca3', subject_id: 'math', class_id: 'c1', stream_id: 'str2', periods_per_week: 5, is_double: false },
    { id: 'sca4', subject_id: 'eng', class_id: 'c1', stream_id: 'str2', periods_per_week: 4, is_double: false }
  ],
...
async function run() {
  ...
  // ---- requirements (periods/week + double flag on subject_class_assignments) --
  {
    const { timetable, sb } = freshApis();
    const res = await timetable.requirements.save('sca1', 6, true);
    check('requirements.save succeeds', res.ok === true);
    const row = sb._tables.subject_class_assignments.find((r) => r.id === 'sca1');
    check('requirements.save updates periods_per_week and is_double on the right row', row.periods_per_week === 6 && row.is_double === true);
    check('requirements.save requires an assignment id', (await timetable.requirements.save(null, 5, false)).ok === false);
  }
