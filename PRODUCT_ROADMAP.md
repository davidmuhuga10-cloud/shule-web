...
- **Attendance** — daily marking (student + staff), history, basic reports, simple correlation view against exam performance.
- **Messaging/SMS** — integration with a Kenyan SMS gateway (Africa's Talking is the standard choice for this market), credit/wallet system, targeted sending (class/stream/individual), delivery logging, and an opt-out mechanism (even if simpler than Zeraki's USSD code — an SMS-reply keyword is achievable now, USSD requires a telco relationship we don't have yet).
- **Parent Portal** — a new account kind (Section 6) that can log in and see their child(ren)'s report cards, attendance, and messages. This is pure retention/word-of-mouth fuel — parents asking "why don't we have this app" is a powerful sales channel we don't have yet.
...
2. **Up next: Phase 1 (Communication & Presence Parity)** — Attendance, Messaging/SMS, Parent Portal, plus the Section 6 role/permission redesign that Phase 0 deliberately deferred. I'll scope and build this the same way: verified in a sandbox (or simulated Postgres instance) before it touches your live data, real unit tests per module, real browser verification per screen.
3. This document stays the living source of truth — each phase's section gets a status update like Phase 0's above as it ships, rather than a separate changelog.
