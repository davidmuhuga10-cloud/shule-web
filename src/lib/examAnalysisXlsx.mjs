...
function gradeSummaryRows(title, rows, bandLabels) {
  ...
  rows.forEach((r) => {
    out.push([r.label || r.subject_name, ...bandLabels.map((l) => r.band_counts[l] || 0), r.x_count, r.entries, r.mean_marks, r.mean_points, r.performance_level]);
  });
  out.push([]);
  return out;
}
...
export function buildExamAnalysisAoa({ settings, exam, cls, analysis }) {
  ...
  aoa.push(['Mean Marks', analysis.mean_marks]);
  aoa.push(['Mean Points', analysis.mean_points]);
  ...
  analysis.learning_area_stats.forEach((r) => aoa.push([r.name, r.points, r.performance_level]));
  aoa.push([]);
