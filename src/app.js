...
// ROUND 7 (mobile/app "Download PDF" removed entirely): this app went
// through several rounds of a separate, one-tap "Download PDF" button on
// phone widths — html2canvas rasterizing the page into an image, then jsPDF
// laying that onto real pages — chasing a different failure almost every
// time it seemed fixed (blur, overlapping text, a missing border, then a
// large class running out of memory, then a fresh "too many..." failure
// that a real reproduction against the exact deployed libraries couldn't
// even confirm, then Finance's own Download coming back with only the
// header and nothing else). Explicit live feedback after that last round:
// "I THINK WE CAN JUST DO AWAY WITH THIS DOWNLOAD ON PHONE AND APP
// COMPLETELY LETS JUST HAVE PRINT ONLY." Print never had any of these
// problems — it hands the page straight to the platform's own print
// pipeline (window.print()'s "Save as PDF" in a browser, Android's real
// PrintManager sheet in the native app via @capgo/capacitor-printer) instead
// of rebuilding a PDF by hand, so it's kept as the one, only way to get a
// PDF everywhere, on every screen. The old Download button, and every
// function that only existed to support it (wireDownloadPdf and everything
// below it in this file), are gone.
const PRINT_BTN_LABEL_HTML = '🖨️ Print';
